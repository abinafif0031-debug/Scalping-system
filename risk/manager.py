...
- Per-session trade limits (pre-market vs open)
...
class RiskManager:
        ...
        try:
            if os.path.exists(STATE_FILE):
                with open(STATE_FILE) as f:
                    data = json.load(f)
                # Support old state files missing new fields
                for field in ["pre_market_trades", "open_market_trades"]:
    ...
    def can_trade(self, session: str = "open") -> tuple[bool, str]:
        """
        session: "pre" or "open"
        ...
        if session == "pre":
            if self.state.pre_market_trades >= MAX_TRADES_PRE_MARKET:
                return False, f"Pre-market limit reached ({MAX_TRADES_PRE_MARKET} trades)"
        else:
            if self.state.open_market_trades >= MAX_TRADES_OPEN_MARKET:
                return False, f"Open market limit reached ({MAX_TRADES_OPEN_MARKET} trades)"
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "pre_market_trades":   f"{self.state.pre_market_trades}/{MAX_TRADES_PRE_MARKET}",
            "open_market_trades":  f"{self.state.open_market_trades}/{MAX_TRADES_OPEN_MARKET}",
